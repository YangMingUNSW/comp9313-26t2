...
description: Two-file output convention for every COMP9313 assignment solution
metadata:
  node_type: memory
  type: feedback
---

For every COMP9313 assignment, produce the solution in TWO files (do not delete the readable one):
...
**Why:** the user submits PDFs built in Overleaf but keeps the repo Markdown-only; the `_pdf` suffix tells them which file to paste into Overleaf.
